...
router.get("/add-product", (req, res, next) => {
  res.send(
    '<html><body><form action="/admin/add-product" method="POST"><label>Product Name :</label><input type="text" name="product" /><br/><label>Size</label><input type="number" name="size" /><button>Add Product</button></form></body></html>'
  );
});
